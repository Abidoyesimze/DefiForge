...
const DeFiUtilsPage = () => {
  ...
  const calculateLiquidity = async () => {
    if (!validateInput(token0Amount, "Token 0 Amount") || 
  ...
  const calculateImpermanentLoss = async () => {
    if (!validateInput(initialPrice, "Initial Price") || 
        !validateInput(finalPrice, "Final Price")) {
      return;
    }
  ...
  const calculateSwapFee = async () => {
    if (!validateInput(amountIn, "Amount In") || 
        !validateInput(feePercentage, "Fee Percentage")) {
      return;
    }
        ...
        {!isConnected ? (
          <div className="text-center py-12">
            ...
            <p className="text-gray-300 mb-6">
              Please connect your wallet to Somnia testnet to use DeFi utilities.
            </p>
          </div>
        ) : (
          <>
                ...
                <button
                  ...
                  className={`px-6 py-3 mx-2 mb-2 rounded-lg font-medium transition-colors ${
                    activeTab === tab.id
                      ...
                      : "bg-[#1c2941] text-gray-300 hover:bg-[#243a5f]"
                  }`}
                >
        ...
        <div className="mt-12 text-center">
          <div className="bg-[#1c2941] p-6 rounded-lg max-w-2xl mx-auto">
            ...
            <p className="text-sm text-gray-300 mb-2">
              DeFi Utils deployed at:
            </p>
            <code className="text-purple-400 text-sm break-all">
              {CONTRACT_ADDRESSES.DeFiUtils}
            </code>
          </div>
        </div>
      </div>
