...
describe("MerkleProof", function () {
  ...
  describe("Merkle Tree Management", function () {
    it("Should add a new Merkle tree successfully", async function () {
      ...
      const tx = await merkleProof.connect(user1).addMerkleTree(
        merkleRoot,
        description,
        BigInt(listSize),
      ...
      const event = receipt?.logs.find(
        (log: any) => log.fragment?.name === "MerkleTreeAdded"
      );
      ...
      if (event) {
        ...
        if (parsedEvent) {
          ...
          expect(parsedEvent.args[2]).to.equal(BigInt(listSize));
          expect(parsedEvent.args[3]).to.equal(user1Address);
        }
      }
    });
    ...
    it("Should allow newcomers to add first tree for free", async function () {
      ...
      await expect(
        merkleProof.connect(user1).addMerkleTree(merkleRoot, description, BigInt(listSize))
      ).to.not.be.reverted;
    });
    ...
    it("Should require fee for non-newcomers", async function () {
      ...
      await expect(
        merkleProof.connect(user1).addMerkleTree(newRoot, "Second tree", BigInt(listSize), { value: ethers.parseEther("0.001") })
      ).to.not.be.reverted;
    });
    ...
    it("Should fail with insufficient fee", async function () {
      ...
      await merkleProof.connect(user2).addMerkleTree(
        "0x" + "2".repeat(64),
        "First tree for user2",
        BigInt(listSize)
      );
    ...
    it("Should fail with zero list size", async function () {
      ...
      await expect(
        merkleProof.connect(user1).addMerkleTree(
          merkleRoot,
          description,
          0n,
          { value: ethers.parseEther("0.001") }
  ...
  describe("Merkle Tree Information", function () {
    ...
    beforeEach(async function () {
      ...
      // Add a test tree
      await merkleProof.connect(user1).addMerkleTree(
        testRoot,
        "Test whitelist",
        BigInt(3)
      );
  ...
  describe("Merkle Tree Updates", function () {
    ...
    it("Should allow owner to update any tree description", async function () {
      ...
      await expect(
        merkleProof.connect(owner).updateMerkleTreeDescription(testRoot, newDescription)
      ).to.be.revertedWith("Only creator can update");
    });
    ...
    it("Should fail when non-creator non-owner tries to update description", async function () {
      ...
      await expect(
        merkleProof.connect(user2).updateMerkleTreeDescription(testRoot, newDescription)
      ).to.be.revertedWith("Only creator can update");
    });

    it("Should fail with empty description", async function () {
      await expect(
        merkleProof.connect(user1).updateMerkleTreeDescription(testRoot, "")
      ).to.be.revertedWith("Description cannot be empty");
    });
    ...
    it("Should emit description update event", async function () {
      ...
      const event = receipt?.logs.find(
        (log: any) => log.fragment?.name === "MerkleTreeUpdated"
      );
  ...
  describe("Merkle Tree Removal", function () {
    ...
    it("Should allow owner to remove any tree", async function () {
      ...
      await expect(
        merkleProof.connect(owner).removeMerkleTree(testRoot)
      ).to.be.revertedWith("Only creator can remove");
    });

    it("Should fail when non-creator non-owner tries to remove tree", async function () {
      await expect(
        merkleProof.connect(user2).removeMerkleTree(testRoot)
      ).to.be.revertedWith("Only creator can remove");
    });
    ...
    it("Should emit tree removal event", async function () {
      ...
      const event = receipt?.logs.find(
        (log: any) => log.fragment?.name === "MerkleTreeRemoved"
      );
  ...
  describe("Fee Management", function () {
    ...
    it("Should fail when non-treasury tries to update platform fee", async function () {
      ...
      await expect(
        merkleProof.connect(user1).updatePlatformFee(newFee)
      ).to.be.revertedWith("Only treasury can update fee");
    });
    ...
    it("Should emit fee update event", async function () {
      ...
      const event = receipt?.logs.find(
        (log: any) => log.fragment?.name === "PlatformFeeUpdated"
      );
  ...
  describe("Edge Cases", function () {
    it("Should handle very long descriptions", async function () {
      ...
      await expect(
        merkleProof.connect(user1).addMerkleTree(testRoot, longDescription, BigInt(listSize))
      ).to.not.be.reverted;
  ...
  describe("Security Features", function () {
    it("Should prevent unauthorized tree modifications", async function () {
      ...
      await expect(
        merkleProof.connect(user2).updateMerkleTreeDescription(testRoot, "Unauthorized")
      ).to.be.revertedWith("Only creator can update");
    ...
    it("Should prevent fee manipulation", async function () {
      ...
      await expect(
        merkleProof.connect(user1).updatePlatformFee(ethers.parseEther("0.002"))
      ).to.be.revertedWith("Only treasury can update fee");
